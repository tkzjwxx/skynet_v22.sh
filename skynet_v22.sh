#!/bin/bash
# ====================================================================
# 天网系统 V22+ (终极基因克隆 + 极速沙盒测网版)
# ====================================================================
clear
echo -e "\033[1;36m=================================================================\033[0m"
echo -e "\033[1;37m                 🛡️ 天网系统 V22+ (全境克隆重构版) 🛡️\033[0m"
echo -e "\033[1;36m=================================================================\033[0m"
...
echo -e "\033[1;36m-----------------------------------------------------------------\033[0m"
echo -e "  \033[1;32m[1]\033[0m 🚀 嗅探环境并部署天网 (完全独立运行，绝不破坏原版文件)"
...
clear
echo -e "\033[1;31m🔥 正在执行【天网 V22+】环境克隆与重构部署...\033[0m"
...
[ "$CORE_DIR" == "/" ] && CORE_DIR="/etc/s-box" # 容错回退

if [ -z "$CORE_FILE" ] || [ ! -f "$CORE_FILE" ]; then
    echo -e "\033[1;31m[失败]\033[0m\n❌ 无法定位核心文件物理路径！"
    exit 1
fi
echo -e "\033[1;32m[通过] (核心: $CORE_FILE, 账号环境: $CORE_DIR)\033[0m"
sleep 2

# ====================================================================
# 2. 基因克隆：建立绝对隔离的 /etc/skynet 基地
# ====================================================================
echo -e "\n\033[1;33m[阶段 2] 正在进行无损克隆与沙盒裂变...\033[0m"

# 创建独立的天网基地，绝不破坏勇哥的 /etc/s-box
rm -rf /etc/skynet
mkdir -p /etc/skynet/sub1 /etc/skynet/sub2 /etc/skynet/sub3 /etc/skynet/sub4 /etc/skynet/blacklist

# 将勇哥的原版环境（包含账号、证书等）完整克隆过来，确保核心能启动！
cp -a "$CORE_DIR/." /etc/skynet/sub1/
cp -a "$CORE_DIR/." /etc/skynet/sub2/
cp -a "$CORE_DIR/." /etc/skynet/sub3/
cp -a "$CORE_DIR/." /etc/skynet/sub4/
...
PORT_S1=40000; PORT_S2=40001; PORT_S3=40002
...
echo "us.domain.com" > /etc/skynet/cf_s1.info
echo "uk.domain.com" > /etc/skynet/cf_s2.info
echo "jp.domain.com" > /etc/skynet/cf_s3.info
echo "$VLESS_UUID" > /etc/skynet/vless_uuid.info
echo "$USER_PASS" > /etc/skynet/hy2_pass.info

# 停止原版占用以释放 40000 端口
systemctl stop sing-box 2>/dev/null; systemctl disable sing-box 2>/dev/null
kill -9 $TARGET_PID 2>/dev/null

# 拉取官方纯正的 sing-box 作为前端接客门面
echo -e "⏳ 正在拉取纯净前端核心 (Sing-box)..."
...
tar -xzf /tmp/sbox.tar.gz -C /tmp/ && mv /tmp/sing-box-*/sing-box /etc/skynet/front-box
chmod +x /etc/skynet/front-box
...
for NODE in 1 2 3; do
    [ "$NODE" == "1" ] && { IN_PORT=2081; OUT_PORT=1081; DIR="/etc/skynet/sub1"; }
    [ "$NODE" == "2" ] && { IN_PORT=2082; OUT_PORT=1082; DIR="/etc/skynet/sub2"; }
    [ "$NODE" == "3" ] && { IN_PORT=2083; OUT_PORT=1083; DIR="/etc/skynet/sub3"; }

    cat << EOF > /etc/skynet/sl${NODE}
#!/bin/bash
NODE="${NODE}"; IN_PORT="${IN_PORT}"; OUT_PORT="${OUT_PORT}"; WORK="${DIR}"; SLA_LOG="/etc/skynet/stability.log"
...
while true; do
    ...
    # 将沙盒建立在 /tmp，彻底断绝嵌套死锁
    RUN="/tmp/skynet_run_\$RANDOM"
    mkdir -p \$RUN
    # 【核心修复】将带有账号的根目录完全克隆进去！
    cp -a "\$WORK/." "\$RUN/" 2>/dev/null
    ...
    nohup ./sbwpph -b 127.0.0.1:\$IN_PORT --cfon --country \$(cat \$WORK/region.info) -4 --endpoint "\$EP" >/dev/null 2>&1 & disown
...
find /etc/skynet -name "*.manual" -o -name "*.session" -o -name "*.hibernating" -o -name "*.boot" | xargs rm -f 2>/dev/null
echo "$(date '+[%m-%d %H:%M:%S]') 🚀 VPS 开机/重置！天网哨兵就绪，准备引导快照回档。" >> "$SLA_LOG"

while true; do
    for NODE in 1 2 3; do
        [ "$NODE" == "1" ] && { IN_PORT=2081; OUT_PORT=1081; WORK="/etc/skynet/sub1"; }
        [ "$NODE" == "2" ] && { IN_PORT=2082; OUT_PORT=1082; WORK="/etc/skynet/sub2"; }
        [ "$NODE" == "3" ] && { IN_PORT=2083; OUT_PORT=1083; WORK="/etc/skynet/sub3"; }
        ...
        if [[ -n "$CURRENT" && "$CURRENT" == "$TARGET" ]]; then
        ...
        elif ! pgrep -f "/etc/skynet/sl${NODE}" > /dev/null; then
            if [ -z "$CURRENT" ]; then
                sleep 15
                CURRENT=$(curl -s --max-time 10 --socks5-hostname 127.0.0.1:$IN_PORT $API 2>/dev/null | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | head -n 1)
            fi
            ...
            if [[ -n "$CURRENT" && "$CURRENT" != "$TARGET" ]] || [ -z "$CURRENT" ]; then
                ...
                nohup /etc/skynet/sl${NODE} >/dev/null 2>&1 &
                sleep 10
            fi
        fi
    done
...
get_node_vars() {
    case $1 in
        1) N_IN=2081; N_OUT=1081; N_DIR="/etc/skynet/sub1"; N_SVC="skynet-s1" ;;
        2) N_IN=2082; N_OUT=1082; N_DIR="/etc/skynet/sub2"; N_SVC="skynet-s2" ;;
        3) N_IN=2083; N_OUT=1083; N_DIR="/etc/skynet/sub3"; N_SVC="skynet-s3" ;;
    esac
...
action_draw() {
    ...
    while true; do
        ...
        RUN="/tmp/skynet_run_$RANDOM"
        mkdir -p "$RUN"
        # 👑【核心修复】带上包含账号的家产去沙盒运行，绝不暴毙！
        cp -a "$N_DIR/." "$RUN/" 2>/dev/null
        cd "$RUN"; export HOME="$RUN"
        
        EP=${ENDPOINTS[$RANDOM % ${#ENDPOINTS[@]}]}
        echo "$EP" > "current.endpoint"
        
        echo -ne "\r\033[K\033[1;36m⏳ 沙盒建毕，端点 ($EP) 盲抽中...\033[0m"
        ...
        IP=""
        for i in {1..4}; do
            IP=$(curl -s --max-time 6 --socks5-hostname 127.0.0.1:$N_IN ${APIS[$RANDOM % ${#APIS[@]}]} 2>/dev/null | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | head -n 1)
            if [ -n "$IP" ]; then break; fi
            echo -ne "\r\033[K\033[1;35m⏳ 端点 ($EP) 握手中，第 $i 次重试测网...\033[0m"
            sleep 2
        done
        ...
        if [[ "$k" == "y" || "$k" == "Y" ]]; then
            ...
            cp -a "$RUN/." "$N_DIR/golden_snapshot/" 2>/dev/null
            rm -rf "$RUN"
            ...
            cp -a "$N_DIR/golden_snapshot/." "$N_DIR/" 2>/dev/null
...
action_toggle() {
    ...
    if [ -f "$N_DIR/s$N.disabled" ]; then
        ...
        fuser -k -9 "$N_OUT/tcp" >/dev/null 2>&1
        pkill -f "/etc/skynet/sl$N" 2>/dev/null
...
# ====================================================================
# 🔥 幽灵斥候 S4：深海打捞旁路引擎 (纯正克隆版)
# ====================================================================
action_s4() {
    DIR="/etc/skynet/sub4"; BLACKLIST_FILE="/etc/skynet/blacklist/bad_ips.txt"; SVC="skynet-s4"; IN_PORT=2084; touch "$BLACKLIST_FILE"
    ...
    if [ "$c" == "1" ]; then
        ...
        while [ $A -lt $SCAN_MAX ]; do
            ...
            mkdir -p "$RUN"
            # 👑【核心修复】同样把老家财产克隆进沙盒
            cp -a "$DIR/." "$RUN/" 2>/dev/null
            cd "$RUN"; export HOME="$RUN"
            
            EP=${ENDPOINTS[$RANDOM % ${#ENDPOINTS[@]}]}
            echo "$EP" > "current.endpoint"
            
            nohup ./sbwpph -b 127.0.0.1:$IN_PORT --cfon --country $TR -4 --endpoint "$EP" >/dev/null 2>&1 & disown
...
echo -e "\n\033[1;32m🎉 天网系统 V22+ 终极版部署完毕！所有底层依赖与环境已无损隔离！\033[0m"
echo -e "\033[1;37m👉 请在终端输入 \033[1;36mtw\033[1;37m 开始享受纯正的赛风抽卡！\033[0m"
